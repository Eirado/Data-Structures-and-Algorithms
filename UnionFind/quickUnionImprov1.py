...
class QuickUnion:
    def __init__(self, entries: list):
        self.id = entries
        self.size = [1] * len(entries)
    def is_connected(self,p: int, q: int) -> bool:
        if self.find_root(p) == self.find_root(q):
            return True
        else:
            return False
    def find_root(self, x: int): # 2
        while self.id[x] != x:
            self.id[x] = self.id[self.id[x]] # path compression (it point to its grandfather) cut half of the path
            x = self.id[x]
        return x

    def unionUF(self, p: int, q: int): #4,3
